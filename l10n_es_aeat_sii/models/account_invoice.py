...
class AccountInvoice(models.Model):
    ...
    def _get_sii_taxes_map(self, codes):
        ...
        tax_templates = sii_map.sudo().with_context(
            force_company=self.company_id.id).map_lines.filtered(
    ...
    def _send_invoice_to_sii(self):
        for invoice in self.filtered(lambda i: i.state in ['open', 'paid']):
            ...
            try:
                ...
                invoice.sii_header_sent = header
                invoice.sii_content_sent = inv_dict
                if invoice.type in ['out_invoice', 'out_refund']:
        ...
        for invoice in invoices:
            ...
            if not company.use_connector:
                ...
                invoice.sudo().with_context(
                    force_company=self.company_id.id).invoice_jobs_ids |= \
                    queue_ids
